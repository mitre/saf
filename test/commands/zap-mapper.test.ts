...
describe('Test zap', () => {
  ...
  test
  .stdout()
  .command(['convert:zap', '-j', path.resolve('./test/sample_jsons/zap/sample_input_report/webgoat.json'), '-n', 'http://mymac.com:8191', '-o', `${tmpobj.name}/zaptest-webgoat.json`])
  ...
  test
  .stdout()
  .command(['convert:zap', '-j', path.resolve('./test/sample_jsons/zap/sample_input_report/zero.webappsecurity.json'), '-n', 'http://zero.webappsecurity.com', '-o', `${tmpobj.name}/zaptest-webappsecurity.json`])
