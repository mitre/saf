...
describe('Test zap', () => {
  ...
  .it('hdf-converter output test - webgoat', () => {
    ...
    const sample = JSON.parse(fs.readFileSync(path.resolve('./test/sample_data/zap/zap-webgoat-hdf.json'), {encoding: 'utf-8'}))
  ...
  .it('hdf-converter output test - zero.webappsecurity', () => {
    ...
    const sample = JSON.parse(fs.readFileSync(path.resolve('./test/sample_data/zap/zap-webappsecurity-hdf.json'), {encoding: 'utf-8'}))
    expect(omitVersions(test)).to.eql(omitVersions(sample))
  })
})
