...
describe('Test zap withraw flag', () => {
  const tmpobj = tmp.dirSync({unsafeCleanup: true})

  test
  .stdout()
  .command(['convert zap2hdf', '-i', path.resolve('./test/sample_data/zap/sample_input_report/webgoat.json'), '-n', 'http://mymac.com:8191', '-o', `${tmpobj.name}/zaptest-webgoat.json`, '-w'])
  ...
  test
  .stdout()
  .command(['convert zap2hdf', '-i', path.resolve('./test/sample_data/zap/sample_input_report/zero.webappsecurity.json'), '-n', 'http://zero.webappsecurity.com', '-o', `${tmpobj.name}/zaptest-webappsecurity.json`, '-w'])
