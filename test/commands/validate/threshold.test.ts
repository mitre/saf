...
describe('Test validate threshold - using inline values', () => {
  ...
  it('Validate threshold test - Invalid inline content', async () => {
    ...
    expect(stderr.replaceAll(/\n/gi,' ').replaceAll(/\t/gi,' ').replaceAll(/\s+/g, ' ').trim()).to.equal('Error: failed.medium.min: Threshold not met. Number of received total failed controls (87) is less than your set threshold for the number of failed controls (97)')
  })
})
