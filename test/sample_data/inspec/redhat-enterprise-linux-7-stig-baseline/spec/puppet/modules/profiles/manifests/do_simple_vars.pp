define profiles::do_simple_vars ( $file, $lens, $context=undef, $set_comment=true ) {
  ...
  if $value {
    augeas { "Insert ${option} after existing comment in ${file}":
      incl    =>  $file,
    ...
    if $set_comment {
      augeas { "Set comment for ${option} in ${file}":
        ...
        require =>  Augeas["Insert ${option} after existing comment in ${file}"],
      }
    } else {
    ...
    augeas { "Remove ${option} from ${file}":
      incl    =>  $file,
      context =>  "/files/${bcontext}",
      lens    =>  $lens,
      changes =>  "rm ${option}",
