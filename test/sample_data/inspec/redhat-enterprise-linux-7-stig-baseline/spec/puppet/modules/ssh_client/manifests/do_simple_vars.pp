define ssh_client::do_simple_vars ( $file, $lens, $context=undef, $set_comment=true ) {
  ...
  augeas { "Insert $option ($bcontext) after comment":
    incl    =>  "$file",
  ...
  augeas { "Set $option ($bcontext) in $file":
    incl    =>  "$file",
    context =>  "/files/$bcontext",
    lens    =>  "$lens",
  ...
  if $set_comment {
    augeas { "Set comment for $option ($bcontext) in $file":
      incl    =>  "$file",
      context =>  "/files/$bcontext",
      lens    =>  "$lens",
      changes =>  [
      ...
      require =>  Augeas["Insert $option ($bcontext) after comment"],
    }
  } else {
    augeas { "Set comment for $option ($bcontext) in $file":
      incl    =>  "$file",
