...
export default class Summary extends Command {
  ...
  static flags = {
    ...
    format: Flags.string({
      char: 'f',
      required: false,
      description: 'Output format (cli, json, yaml, markdown)',
      default: 'cli',
    }),
  ...
  async run() {
    ...
    flags.input.forEach(file => {
      execJSONs[file] = convertFileContextual(
        fs.readFileSync(file, 'utf8'),
      ) as ContextualizedEvaluation
    })
    Object.entries(execJSONs).forEach(([, parsedExecJSON]) => {
      ...
      for (const [severity, severityTargets] of Object.entries(
        ...
        const severityStatusCounts = extractStatusCounts(
          parsedProfile,
          severity,
        )
    ...
    Object.entries(totals).forEach(([profileName, profileValues]: any) => {
      printableSummaries.push({
        ...
        resultSets: Object.entries(execJSONs)
          .filter(([, execJSON]) => {
    ...
    // Check the output format and print the summary
    let outputData
    switch (flags.format) {
      case 'json': {
        outputData = JSON.stringify(printableSummaries)
        break
      }

      case 'yaml': {
        outputData = YAML.stringify(printableSummaries)
        break
      }

      case 'markdown': {
        outputData = markdownSummary(printableSummaries)
        break
      }

      default: {
        outputData = printableSummaries.toString() // or any other way you want to format it for CLI
        break
      }
    }
    ...
    // Write the summary to a file if the output flag is set
    if (flags.output) {
      fs.writeFileSync(flags.output, outputData)
    }
  }
}
