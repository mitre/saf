# -*- encoding : utf-8 -*-
control "V-94843" do
  title "The Red Hat Enterprise Linux operating system must be configured so that the x86 
...
reduced because the user will be prompted before any action is taken."
  desc  "rationale", ""
  desc "check", "Note: If the operating system does not have a graphical user interface 
installed, this requirement is Not 
Applicable.

Verify the operating system is not 
configured to reboot the system when Ctrl-Alt-Delete is 
pressed.

Check that the ctrl-alt-del.target is 
masked and not active in the graphical user interface with the following 
command:
...
If 
\"logout\" is not set to use two single quotations, or is missing, this is a finding." 
  desc "fix", "Configure the system to disable the Ctrl-Alt-Delete sequence for the graphical 
...
Add the setting to disable the Ctrl-Alt-Delete 
sequence for the graphical user interface:
...
logout=''" 
  ...
  tag gid: "V-94843"
  tag rid: "SV-104673r2_rule "
  tag stig_id: "RHEL-07-020231"
  tag fix_id: "F-100967r2_fix "
  tag cci: ["CCI-000366"]
  tag nist: ["CM-6 b"]
