...
Note: If the system does 
not have GNOME installed, this requirement is Not 
Applicable.

Check to see if GNOME is configured 
to display a screensaver after a 15 minute delay with the following 
command:
...
If the \"idle-delay\" setting is missing or 
is not set to \"900\" or less, this is a finding." 
  desc "fix", "Configure the operating system to initiate a screensaver after a 15-minute 
period of inactivity for graphical user 
interfaces.

Create a database to contain the 
system-wide screensaver settings (if it does not already exist) with the following 
command:
...
Edit 
/etc/dconf/db/local.d/00-screensaver and add or update the following lines:
...
You must include the \"uint32\" along with 
the integer key values as shown.
