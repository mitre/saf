...
remote access to privileged accounts via pluggable authentication modules 
(PAM).

Check the \"/etc/sssd/sssd.conf\" file 
for the authentication services that are being used with the following 
command:
...
If the \"pam\" service is not present on all 
\"services\" lines, this is a finding." 
  desc "fix", "Configure the operating system to implement multifactor authentication for 
remote access to privileged accounts via pluggable authentication modules 
(PAM).

Modify all of the services lines in 
\"/etc/sssd/sssd.conf\" or in configuration files found under \"/etc/sssd/conf.d\" to 
include pam." 
