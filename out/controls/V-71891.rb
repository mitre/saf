...
The screen program must be installed to lock sessions on the 
console.
...
Check to see if the screen lock is 
...
If 
the \"lock-enabled\" setting is missing or is not set to \"true\", this is a finding." 
...
Edit 
the \"[org/gnome/desktop/screensaver]\" section of the database file and add or update the 
following lines:

# Set this to true to lock the 
screen when the screensaver activates
lock-enabled=true
