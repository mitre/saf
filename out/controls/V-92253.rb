...
networks, but is helpful for end hosts and routers serving small networks."
  desc  "rationale", ""
  desc "check", "Verify the system uses a reverse-path filter for 
IPv4:
...
If \"net.ipv4.conf.default.rp_filter\" is 
not configured in the /etc/sysctl.conf file or in the /etc/sysctl.d/ directory, is 
commented out, or does not have a value of \"1\", this is a 
finding.

Check that the operating system 
implements the accept source route variable with the following 
command:
