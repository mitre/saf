...
configured.
  ...
  "
  desc  "rationale", ""
  desc "check", "Verify the operating system has the screen package 
installed.

Check to see if the screen package is 
...
If 
the screen package is not installed, check to see if the tmux package is installed with the 
following command:
...
If either the screen package or the tmux package 
...
Install the 
screen program (if it is not on the system) with the following 
command:
...
Install 
the tmux program (if it is not on the system) with the following 
command:
