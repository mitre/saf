...
Check to see what level 
\"auditctl\" is set to with following command: 
...
Note: If the value of \"failure\" is set to 
\"2\", the system is configured to panic (shut down) in the event of an auditing failure. If the 
value of \"failure\" is set to \"1\", the system is configured to only send information to the 
kernel log regarding the failure.

If the 
\"failure\" setting is set to any value other than \"1\" or \"2\", this is a 
finding.

If the \"failure\" setting is not set, 
this should be upgraded to a CAT I finding.

If the 
\"failure\" setting is set to \"1\" but the availability concern is not documented or there is 
no monitoring of the kernel log, this should be downgraded to a CAT III finding." 
  desc "fix", "Configure the operating system to shut down in the event of an audit processing 
failure.

Add or correct the option to shut down 
...
Edit the 
\"/etc/audit/rules.d/audit.rules\" file and add the following 
line:

-f 
2

If availability has been determined to be more 
important, and this decision is documented with the ISSO, configure the operating system to 
notify system administration staff and ISSO staff in the event of an audit processing failure 
...
Kernel log monitoring must also be configured 
to properly alert designated staff.
