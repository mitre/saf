...
check to see a session lock occurs when the screensaver is activated with the following 
command:
...
If the \"lock-delay\" setting is missing, or is 
not set to \"5\" or less, this is a finding." 
  desc "fix", "Configure the operating system to initiate a session lock for graphical user 
interfaces when a screensaver is 
activated.
...
Add 
the setting to enable session locking when a screensaver is activated:

[org/gnome/desktop/screensaver]
lock-delay=uint32 
5

The \"uint32\" must be included along with the 
...
Users must log out and back in again before 
