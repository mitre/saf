...
successful/unsuccessful attempts to use the \"chown\" syscall 
occur.
...
-a always,exit -F 
arch=b32 -S chown -F auid>=1000 -F auid!=4294967295 -k 
perm_mod

-a always,exit -F arch=b64 -S chown -F 
auid>=1000 -F auid!=4294967295 -k perm_mod

If 
both the \"b32\" and \"b64\" audit rules are not defined for the \"chown\" syscall, this is a 
finding." 
  desc "fix", "Add or update the following rule in 
\"/etc/audit/rules.d/audit.rules\":

-a 
always,exit -F arch=b32 -S chown -F auid>=1000 -F auid!=4294967295 -k 
perm_mod
