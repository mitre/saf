...
Check if \"SELinux\" is active and in 
\"Enforcing\" mode with the following 
command:
...
If 
\"SELinux\" is not active and not in \"Enforcing\" mode, this is a finding." 
  desc "fix", "Configure the operating system to verify correct operation of all security 
functions.

Set the \"SELinux\" status and the 
\"Enforcing\" mode by modifying the \"/etc/selinux/config\" file to have the following line:
