...
# grep 
kernel.randomize_va_space /etc/sysctl.conf 
/etc/sysctl.d/*

kernel.randomize_va_space = 
2

If \"kernel.randomize_va_space\" is not 
configured in the /etc/sysctl.conf file or in the /etc/sysctl.d/ directory, is commented 
out or does not have a value of \"2\", this is a 
finding.

Check that the operating system 
implements virtual address space randomization with the following 
command:
...
If \"kernel.randomize_va_space\" does not 
have a value of \"2\", this is a finding." 
  desc "fix", "Configure the operating system implement virtual address space 
randomization.

Set the system to the required 
kernel parameter by adding the following line to \"/etc/sysctl.conf\" or a config file in the 
