...
maintaining the confidentiality of the key used to generate the hash.
  "
  desc  "rationale", ""
  desc "check", "If LDAP is not being utilized, this requirement is Not 
Applicable.

Verify the operating system 
implements cryptography to protect the integrity of remote LDAP access 
sessions.

To determine if LDAP is being used for 
...
Services Daemon
Loaded: loaded 
(/usr/lib/systemd/system/sssd.service; enabled; vendor preset: 
disabled)
Active: active (running) since Wed 2018-06-27 10:58:11 
EST; 1h 50min ago

If the \"sssd.service\" is 
\"active\", then LDAP is being used. 

Determine 
the \"id_provider\" the LDAP is currently 
using:
...
If \"id_provider\" is set to \"ad\", this is 
Not Applicable.

Verify the sssd service is 
configured to require the use of certificates:
...
If the \"ldap_tls_reqcert\" setting is 
missing, commented out, or does not exist, this is a 
finding.

If the \"ldap_tls_reqcert\" setting 
is not set to \"demand\" or \"hard\", this is a finding." 
