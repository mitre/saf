...
to boot into single-user mode or make modifications to the boot menu."
  desc  "rationale", ""
  desc "check", "For systems that use BIOS, this is Not 
Applicable.

For systems that are running a 
version of RHEL prior to 7.2, this is Not 
Applicable.
...
If 
the root password does not begin with \"grub.pbkdf2.sha512\", this is a 
finding.

Verify that the \"root\" account is set 
as the \"superusers\":

# grep -iw 
\"superusers\" /boot/efi/EFI/redhat/grub.cfg
 set 
superusers=\"root\"
 export 
superusers

If \"superusers\" is not set to 
...
Edit the 
/boot/efi/EFI/redhat/grub.cfg file and add or modify the following lines in the \"### BEGIN 
