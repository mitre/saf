...
user, who has a home directory of 
\"/home/smithj\".

# cut -d: -f 1,3,6 
/etc/passwd | egrep \":[1-4][0-9]{3}\"
smithj:1000:/home/smithj

Note: 
This may miss interactive users that have been assigned a privileged User Identifier (UID). 
Evidence of interactive use may be obtained from a number of log files containing system logon 
information.

Check the owner of all local 
...
initialization files are not owned by that user or root, this is a finding." 
  desc "fix", "Set the owner of the local initialization files for interactive users to either 
the directory owner or root with the following 
command:
