...
applications are allowed to communicate over the network.
  ...
  "
  desc  "rationale", ""
  desc "check", "Verify the operating system enabled an application 
firewall.

Check to see if \"firewalld\" is 
...
If 
the \"firewalld\" package is not installed, ask the System Administrator if another 
firewall application (such as iptables) is installed. 


If an application firewall is not installed, 
this is a finding. 

Check to see if the firewall is 
...
(/usr/lib/systemd/system/firewalld.service; enabled)
 Active: 
active (running) since Tue 2014-06-17 11:14:49 CEST; 5 days 
ago

If \"firewalld\" does not show a status of 
\"loaded\" and \"active\", this is a finding. 
...
If \"firewalld\" 
does not show a state of \"running\", this is a finding." 
  desc "fix", "Ensure the operating system's application firewall is 
enabled.

Install the \"firewalld\" package, if 
it is not on the system, with the following 
command:
