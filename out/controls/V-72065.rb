...
Check that a file system/partition 
has been created for \"/tmp\" with the following 
command:
...
If the 
\"tmp.mount\" service is not enabled, check to see if \"/tmp\" is defined in the fstab with a 
device and mount point:

# grep -i /tmp 
/etc/fstab
UUID=a411dc99-f2a1-4c87-9e05-184977be8539 /tmp ext4 
rw,relatime,discard,data=ordered,nosuid,noexec, 0 
0

If \"tmp.mount\" service is not enabled and the 
\"/tmp\" directory is not defined in the fstab with a device and mount point, this is a 
finding." 
  desc "fix", "Start the \"tmp.mount\" service with the following 
command:
...
Edit the 
\"/etc/fstab\" file and ensure the \"/tmp\" directory is defined in the fstab with a device 
and mount point." 
  impact 0.3
  tag severity: "low "
