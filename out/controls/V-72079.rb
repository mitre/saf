...
to establish when (date and time) the events 
occurred.

Check to see if auditing is active by 
...
If 
the \"auditd\" status is not active, this is a finding." 
  desc "fix", "Configure the operating system to produce audit records containing 
information to establish when (date and time) the events 
occurred.

Enable the auditd service with the 
following command:
