...
users to attain unauthorized administrative access."
  desc  "rationale", ""
  desc "check", "Verify that the \"nodev\" option is configured for 
/dev/shm:


# cat 
/etc/fstab | grep /dev/shm
tmpfs /dev/shm tmpfs 
defaults,nodev,nosuid,noexec 0 0

If any 
results are returned and the \"nodev\" option is not listed, this is a 
finding.

Verify \"/dev/shm\" is mounted with 
the \"nodev\" option:
