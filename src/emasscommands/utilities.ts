...
export function getFlagsForEndpoint(argv: string[]) {
  ...
  if (args.requestType === 'get') {
    if (args.endpoint === 'system') {
      ...
      return {
        systemId: Flags.integer({char: "s", description: "The system identification number", required: true}),
        filename: Flags.integer({char: "f", description: "The poam identification number", required: true}),
        compress: Flags.integer({char: "c", description: "Compress true or false", required: true}),
      }
      ...
      return {
        ...
        pageIndex: Flags.string({char: "p", description: "The page number to query", required: false}),
        sinceDate: Flags.string({char: "d", description: "The Workflow Instance date. Unix date format", required: false}),
        status: Flags.string({char: "s", description: "Possible values: active, inactive, all", required: false}),
      }
...
export function getDescriptionForEndpoint(argv: string[]): string {
  ...
  if (args.requestType === 'get') {
    if (args.endpoint === 'roles') {
      ...
      if (args.argument === 'forSystem') {
        return 'Retrieves one or many artifacts in a system specified system ID';
...
export function getExamplesForEndpoint(argv: string[]): string[] {
  ...
  if (args.requestType === 'get' && args.endpoint === 'roles') {
    ...
    if (args.argument === 'forSystem') {
      ...
      return ['emasser get artifacts export --systemId <value>'];
    } else {
      return ['emasser get artifacts forSystem --systemId <value> [options]', 'emasser get artifacts export --systemId <value>'];
    }    
