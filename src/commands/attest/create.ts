...
export default class CreateAttestations extends Command {
  ...
  static flags = {
    format: Flags.string({char: 't', default: 'json', description: '(optional) The output file type', options: ['json', 'xlsx', 'yml', 'yaml']}),
    help: Flags.help({char: 'h'}),
    input: Flags.string({char: 'i', description: '(optional) An input HDF file to search for controls'}),
    output: Flags.string({char: 'o', description: 'The output filename', required: true}),
  }

  static usage = 'attest create -o <attestation-file> [-i <hdf-json> -t <json | xlsx | yml | yaml>]'

  getStatus(): 'failed' | 'passed' { // skipcq: JS-0105
