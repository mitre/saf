...
export default class EmasserContainerScans extends Command {
  ...
  static examples = ['<%= config.bin %> <%= command.id %> [-s,--systemId] [-f,--containerCodeScanFile]',
      'The input file should be a well formed JSON containing the container scan results and their scan results information.',
  ...
  async run(): Promise<void> {
    ...
    let requestBodyArray: ContainerResource[] = [];
    ...
    if(fs.existsSync(flags.containerCodeScanFile)) {
      ...
      if (Array.isArray(data)) {
        data.forEach(function(dataObject: ContainerResource) {
        ...
        let dataObject: ContainerResource = data;
        let bodyObj: ContainerResource = {
          containerId: '',
          containerName: '',
          time: 0,
...
function addRequiredFieldsToRequestBody(dataObj: ContainerResource): ContainerResource {
  ...
  let benchmarksArray: Benchmarks[] = [];
  ...
  try {
    ...
    let i = 0, j = 0;
    dataObj.benchmarks.forEach(function(entryObject: Benchmarks) {
      assertParamExists(`benchmarks[${i}].benchmark`, entryObject.benchmark);

      entryObject.results.forEach(function(resultObj: Results) {
...
function addOptionalFields(bodyObject: ContainerResource, dataObj: ContainerResource): void {
  ...
  if (dataObj.hasOwnProperty('namespace')) { bodyObject.namespace = dataObj.namespace; }
  if (dataObj.hasOwnProperty('podIp')) { bodyObject.podIp = dataObj.podIp; }
  if (dataObj.hasOwnProperty('podName')) { bodyObject.podName = dataObj.podName; }
  ...
  dataObj.benchmarks.forEach(function(entryObject: Benchmarks) {
    ...
    if (entryObject.hasOwnProperty('isBaseline')) { benchmarksObj.isBaseline = entryObject.isBaseline; }
    ...
    entryObject.results.forEach(function(resultObj: Results) {
      ...
      resultsObj.lastSeen = resultObj.lastSeen;
      // Check for the optional entry
      if (resultObj.hasOwnProperty('isBaseline')) { resultsObj.message = resultObj.message; }
      resultsArray.push(resultsObj);
    });
    benchmarksObj.results = resultsArray
    benchmarksArray.push(benchmarksObj);
