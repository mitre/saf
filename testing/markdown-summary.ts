...
function generateRow(row: string, data: any, columnWidths: number): string {
    let values: string[];
    if (row === "Total") {
        values = [
...
async function processData(data: any, output: string | undefined) {
...
if ((argv as any).input) {
    // If an input file was provided, read the data from the file
    let rawData = fs.readFileSync((argv as any).input);
    let data = JSON.parse(rawData.toString());
    processData(data, (argv as any).output);
} else {
    // If no input file was provided, read the data from stdin
    getStdin().then((rawData) => {
